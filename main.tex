...
The code I developed is also available on github \cite{kockahonzaSHPGenieData2023}, it uses C\textt{++}17 features and ROOT 6.12 as available on the GENIE machine at Fermilab used by us.
It heavily uses classes and the features are best described by describing the code.
The main class \verbb{GenieAnalysis} will expect an input file with a \verbb{gst} dataset, has 2 virtual methods to be overriden \verbb{double passesCuts()} and \verbb{void useEntry(double weight)} and 1 method that is expected to be used \verbb{void runAnalysis(int number\_of\_entries)}\footnotemark.
...
The last of which will start an analysis, it will go through the first \verbb{number\_of\_entries} events from the input file and for each \verbb{passesCuts} will be ran, its return value being used as the weight, so if it is 0 that event is not used at all.
...
Specifically I use types to separate interaction types like QE, RES, $\Delta$ resonances and then also all events, each of these would be a type.
...
Specifically it can create 1D histograms for each property and type combination and also 2D histograms showing how some 2 properties correlate, again it can do this separately for any chosen known types.
...
This was mainly used for first looks and testing of the new code (having 5 executables using these) and \verbb{GACLAS6DataPrepare, GACLAS6Data} do the equivalent for the actual CLAS6 data, though this has not been tested much.
...
% Specifically we use the introduced GENIE data generated for the CLAS6 measurements with a carbon target and beam energy of 2.257\si{GeV}.
% We then have 
