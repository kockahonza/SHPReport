...
The newly developed code is also available on github \cite{kockahonzaSHPGenieData2023}, it uses C\textt{++}17 features and ROOT 6.12 as available on the GENIE machine at Fermilab used by us.
...
\paragraph{Primary State} is the closest to the "truth"/physics we get here.
...
\paragraph{Final State with no Rescattering} should in theory and we observe it to be almost the same as PS.
...
\paragraph{Final State with no Rescattering} means we use final state particles and their properties just as in FSR, but do no checking of \verbb{resc}.
...
\paragraph{Detector Like} This is to get some insight into what level of precision we might expect from detector data.
...
Only one such plot is shown, but all the others are very similar and give values of $1.4 \pm 0.05 \si{GeV}$ for the optimal cut thus we use 1.4\si{GeV}.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        Data is calculated directly from the $W$ histogram hence the steps.
        Carbon target and 2.26\si{GeV} beam energy.
...
See \cref{tab:FSI_re} for these results, these are plausible within other literature numbers, \cite{stevendytmanFinalStateInteractions2009} quotes a 20\% chance of a pion being absorbed in a carbon nucleus (20\%, 21\% for \md, \pd\ for our data, other rescattering mechanisms accounting for the rest).
...
\begin{itemize}
    \item Kinematics and pion reconstruction
    \item The 4 different event sets we use
    \item Start presenting data and results, commenting on physics
\end{itemize}
