...
Then when a neutrinos comes in, some interact with these atoms and produce detectable particles.
...
Which is where \efn\ comes in, in order to better interpret data from neutrino detectors and mainly to improve their energy reconstruction (which is becoming a limiting factor on the uncertainties of the results) we need to  understand these neutrino-nucleus interactions better.
However, as there are no monoenergetic neutrino beams it is not possible to experimentally study these effects directly on neutrinos, so instead we study electron-nucleus interactions which are subject to the same quantum field theory interactions.

\subsection{Neutrino-nucleus interactions}
As neutrinos only interact via the weak force, all neutrino-nucleus interactions take place via the \Zz, \Wp and \Wm bosons.
\subsection{Quasi-elastic interactions}

% Neutrino detectors rely on having a large amount of some substance (Argon, water, Chlorine) with with the neutrino then interacts.
% In many detectors (including water Cherenkov type) only the outgoing lepton

% We naturally don't have any control over the solar and atmospheric neutrinos, but for accelerator sources we can to some degree control the generated neutrino energies, but the control is limited and the neutrino flux is orders of magnitude lower \cite{zuberNeutrinoPhysics2020, deloguENUBETExperiment2022}.
% With NBBs we can know the neutrino energies to some level, however 


% All of these sources tend to have a wide energy range however, 
% However none of these sources have a dominant energy, they all have a wide range, but for calculations of mass differences and PMNS matrix elements we need the $E$ values.
% For that reasons experiments are forced to reconstruct the energies of incoming neutrinos based on the observed events.


% in all of these experiments the neutrinos are either from the sun, microwave cosmic radiation or secondary sources from accelerators\cite{zuberNeutrinoPhysics2020}.
% None of these 
